...
def generate_provider_md(p):
    ...
    traffic_str = p.get('trafficFrom', '待核验')
    summary_str = p.get('summary', '')
    suitable_str = p.get('suitableFor', '通用上网')

    content = f"""---
title: "{p['name']} 机场测评：价格、节点、特点与购买前须知"
slug: "{p['slug']}"
date: "2026-09-23"
lastmod: "2026-09-23"
category: "服务测评"
tags: ["provider-review", "{p['name']}", "机场测评", "梯子推荐"]
description: "{p['name']} (TZRank 独立测评)：详细分析 {p['name']} 的价格套餐、节点覆盖、线路优势、优惠码及购买前注意事项。"
...
**{p['name']}**（别名：{p.get('alternateName', p['name'])}) 是一家针对中文用户网络连接需求打造的代理梯子服务商。在 TZRank Labs 的测评体系中，该服务在**{suitable_str}**方面表现出色。

* **全站推荐排名**：No. {p['rank']}
* **参考起始价格**：{price_str}
* **基础流量规格**：{traffic_str}
* **专属优惠码**：`{coupon_str}`（{coupon_note}）
* **官方专属入口**：[直接前往 {p['name']} 官方结算页]({p['inviteURL']})
...
1. **线路质量与稳定性**：{summary_str}
2. **适用应用场景**：{suitable_str}。
